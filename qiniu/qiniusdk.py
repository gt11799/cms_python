...
class Cow(object):
    ...
    @requests_error_handler
    def saveas(self,bucket,url,filename):

        entry_url = urlsafe_b64encode("%s:%s"%(bucket,filename))
        new_url = "%s|saveas/%s"%(url,entry_url)
        sign = signing(self.secret_key, new_url[len("http://"):])
        api_url = "%s/sign/%s:%s"%(new_url,self.access_key,sign)
        # print api_url
        res = urllib2.urlopen(api_url)
        return res.read()
        # return self.api_call(api_url)
        # res = requests.get(url,safe=False)
        # assert res.status_code == 200, res
        # return  res.json()

    # @requests_error_handler
    @gen.coroutine
    def asyncsaveas(self,bucket,url,filename):
        entry_url = urlsafe_b64encode("%s:%s"%(bucket,filename))
        new_url = "%s|saveas/%s"%(url,entry_url)
        sign = signing(self.secret_key, new_url[len("http://"):])
        api_url = "%s/sign/%s:%s"%(new_url,self.access_key,sign)
        ...
        res = yield httpClient.fetch(api_url,method='GET')
    ...
    # @expected_argument_type(2, (basestring, list, tuple))
    @gen.coroutine
    def asyncPut(self,scope,filename,content):
        ...
        httpClient = tornado.httpclient.AsyncHTTPClient()
        print url
        res = yield httpClient.fetch(url,method='POST',body=data)
        raise gen.Return(res.body)
        # raise gen.Return({"key":"baidu.com"})
...
class Bucket(object):
    ...
    def saveas(self,*args):
        print args
        return self.cow.saveas(self.bucket,*args)

    @gen.coroutine
    def asyncPut(self,*args):
        result = yield self.cow.asyncPut(self.bucket,*args)
        raise gen.Return(result)

    @gen.coroutine
    def asyncsaveas(self,*args):
        result = yield self.cow.asyncsaveas(self.bucket,*args)
        raise gen.Return(result)
